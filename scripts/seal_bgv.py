...
def example_bgv_basics():
    parms = EncryptionParameters(scheme_type.bgv)  # type: ignore
    poly_modulus_degree = 8192
    parms.set_poly_modulus_degree(poly_modulus_degree)
    parms.set_coeff_modulus(CoeffModulus.BFVDefault(poly_modulus_degree))  # type: ignore
    parms.set_plain_modulus(PlainModulus.Batching(poly_modulus_degree, 20))  # type: ignore
    context = SEALContext(parms)  # type: ignore

    keygen = KeyGenerator(context)  # type: ignore
    secret_key = keygen.secret_key()
    public_key = keygen.create_public_key()
    relin_keys = keygen.create_relin_keys()

    encryptor = Encryptor(context, public_key)  # type: ignore
    evaluator = Evaluator(context)  # type: ignore
    decryptor = Decryptor(context, secret_key)  # type: ignore

    batch_encoder = BatchEncoder(context)  # type: ignore
