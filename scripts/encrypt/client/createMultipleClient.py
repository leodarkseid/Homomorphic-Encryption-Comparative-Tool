...
def createMultiple(he_type: list[He_type], balances: list[int], numberOfClients: int = 1) -> Dict[CreateMultipleResultDict, any]:
    ...
    for i in range(numberOfClients):
        profiler = Measure()
        c = profiler.measure_memory_time(
            Client, id=i + 1, balance=balances[i], he_type=he_type, desc='Client Initialization')
        client = c["result"]
    ...
    result = {"result": clientsInstance, "time": end_time -
              start_time, "memory": initTotalMemory, "initData": initDATA}
    return result
