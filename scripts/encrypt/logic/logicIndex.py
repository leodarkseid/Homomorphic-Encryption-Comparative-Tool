...
class Logic:
    ...
    def __init__(self, he_type: Literal["BGV", "BFV", "CKKS"],
                 clients: int,
                 balances: list[int]) -> None:
        with console.status('[bold green] Encrypted Client Initialization',
            ...
            self.listOfClient: list[ClientObject] = createMultiple(
                he_type=he_type, numberOfClients=clients, balances=balances)
    ...
    def creditBalance(self, proposedCreditList):
        with console.status(f'[bold green] Processing Encrypted Credit Balance', spinner='aesthetic') as status:
            for i in range(len(proposedCreditList)):
                ...
                encryptedProposedCredit = m.measure_memory_time(
                    self.encryptInt, he_type=self.he_type, HE=c.HE, value=v, desc='Credit Amount Encryption')

                # result = c.balance + encryptedProposedCredit['result']
                result = m.measure_memory_time(
                    addVal, c.balance, encryptedProposedCredit['result'], desc='Encryption Addition')
    ...
    def debitBalance(self, proposedDebitList):
        with console.status(f'[bold green] Processing Encrypted Debit Balance', spinner='aesthetic') as status:
            for i in range(len(proposedDebitList)):
                ...
                # this is supposed to encrypt the value to be added
                encryptedProposedDebit = m.measure_memory_time(
                    self.encryptInt, he_type=self.he_type, HE=c.HE, value=v, desc='Debit Amount Encryption')

                # result = c.balance + encryptedProposedDebit['result']
                result = m.measure_memory_time(
                    subVal, a=c.balance, b=encryptedProposedDebit['result'], desc='Encryption Substraction')
    ...
    def set_credit_score(self, value):
        for i in range(len(value)):
            ...
            result = m.measure_memory_time(
                self.add_creditScore, value=v, HE=c.client.HE, client=c, desc="set enc credit score")
            # if self.he_type == "BFV":
