...
from .index import Client, ClientObject
...
def createMultiple(balances:list[int],numberOfClients:int = 1) -> Dict[CreateMultipleResultDict, any]:
    initTotalTimer = 0
    initTotalMemory = 0
    ...
    for i in range(numberOfClients):
        profiler = Measure()
        c = profiler.measure_memory_time(Client, id=i+1, balance=balances[i], desc='Client Initialization') 
        client = c["result"]
